...
class GotService {
  ...
  // convenience methods
  async get<T> (url: string, opt?: Partial<GotJSONOptions>): Promise<T> {
    return this.got<T>('GET', url, opt)
  }
  async post<T> (url: string, opt?: Partial<GotJSONOptions>): Promise<T> {
    return this.got<T>('POST', url, opt)
  }
  async put<T> (url: string, opt?: Partial<GotJSONOptions>): Promise<T> {
    return this.got<T>('PUT', url, opt)
  }
  async delete<T> (url: string, opt?: Partial<GotJSONOptions>): Promise<T> {
